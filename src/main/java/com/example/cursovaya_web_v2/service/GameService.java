...
public class GameService {
    ...
    @PostConstruct
    public void init() { matrix = new Chips(fillMatrix()); }
    public void refreshMatrix() { matrix = new Chips(fillMatrix()); }
    ...
    private Chip findSixteenthChip(Chip[][] tempMatrix, int x, int y) {
        ...
        for (int[] position : positions) {
            ...
            if (tempX >= 0 && tempX < 4 && tempY >= 0 && tempY < 4) {
                ...
                if (chip.getValue() == 16) return chip;
            }
        }
        return null;
    }
    public void clickAndCheckChip(int x, int y) {
        ...
        if (sixteenthChip != null) swap(tempMatrix, sixteenthChip, selectedChip);
    }
    ...
    private Chip getRandomNeighborChip(Chip[][] tempMatrix, Chip sixteenthChip) {
        ...
        if (x > 0) neighborChips.add(tempMatrix[x - 1][y]);
        if (x < 3) neighborChips.add(tempMatrix[x + 1][y]);
        if (y > 0) neighborChips.add(tempMatrix[x][y - 1]);
        if (y < 3) neighborChips.add(tempMatrix[x][y + 1]);
